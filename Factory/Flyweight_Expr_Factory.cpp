...
Number_Command* Flyweight_Expr_Factory::construct_number_command(int number) {
  ...
  if (search != this->number_commands.end()) { return search->second; }
...
//
Variable_Command* Flyweight_Expr_Factory::construct_variable_command(const std::string& name,
                                                                     int default_value) {

  // See if the variable already exists in the map
  auto search = this->variable_commands.find(name);
  if (search != this->variable_commands.end()) { return search->second; }
