...
//
template<typename T>
Array<T>::Array(void): Array(DEFAULT_SIZE) {
  this->cur_size_ = 0;
}
...
//
template<typename T>
Array<T>::Array(size_t length): data_(new T[length]), cur_size_(length), max_size_(length) {}

//
// Initializing constructor
//
template<typename T>
Array<T>::Array(size_t length, const T& fill): Array(length) {
  this->fill(fill);
}
...
Array<T>::Array(const Array& other):
  data_(new T[other.cur_size_]), cur_size_(other.cur_size_), max_size_(other.cur_size_) {
...
T& Array<T>::operator[](size_t index) {
  if (index >= this->cur_size_) { throw std::out_of_range("index"); }
...
Array<T> Array<T>::slice(size_t begin, size_t end) const {
  ...
  if (begin >= this->cur_size_) { throw std::out_of_range("begin"); }

  if ((end > this->cur_size_) || (end <= begin)) { throw std::out_of_range("end"); }
