...
//
template <typename T, size_t N>
Fixed_Array<T,N>::Fixed_Array(const Fixed_Array<T,N>& other):
  Fixed_Array()
{
...
template <typename T, size_t N>
template <size_t M>
Fixed_Array<T,N>::Fixed_Array(const Fixed_Array<T,M>& other):
  Fixed_Array()
{
	//Copy the buffer up to the max size
	for (size_t index = 0; (index < N) && (index < M); ++index) {
		this->data_[index] = other[index];
	}
}
