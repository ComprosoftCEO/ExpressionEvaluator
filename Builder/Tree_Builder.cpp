...
// Constructor
//
Tree_Builder::Tree_Builder(): last_token_operand(false), inside_expression(false) {}
...
Tree_Builder::~Tree_Builder() {
  ...
  if (this->in_expression()) { this->release_expression_state(); }
}
...
Math_Expr* Tree_Builder::get_expression() {
  if (this->to_free.is_empty()) { throw Expr_Builder::no_expression_exception(); }
...
void Tree_Builder::release_all_expressions() {
  while (!this->to_free.is_empty()) { delete (this->to_free.pop()); }
}
...
void Tree_Builder::release_expression_state() {
  ...
  if (!this->inside_expression) { return; }
  ...
  while (!this->state_stack.is_empty()) {
    ...
    while (!state.expr_stack.is_empty()) { delete (state.expr_stack.pop()); }

    // Free the temporary operator stack
    while (!state.operator_stack.is_empty()) { delete (state.operator_stack.pop()); }
  }
...
void Tree_Builder::start_new_expression() {
  ...
  if (this->in_expression()) { throw Expr_Builder::invalid_action_exception(); }
...
void Tree_Builder::end_expression() {
  ...
  if (!this->state_stack.is_empty()) { throw Expr_Builder::mismatched_parenthesis_exception(); }
...
void Tree_Builder::test_last_token(bool expected_token) const {
  ...
  if (this->last_token_operand != expected_token) { throw Expr_Builder::invalid_infix_exception(); }
}
...
void Tree_Builder::build_add_operator() {
  this->process_operator([]() -> Operator_Node* { return new Add_Node(); });
}
...
void Tree_Builder::build_subtract_operator() {
  this->process_operator([]() -> Operator_Node* { return new Subtract_Node(); });
}
...
void Tree_Builder::build_multiply_operator() {
  this->process_operator([]() -> Operator_Node* { return new Multiply_Node(); });
}
...
void Tree_Builder::build_divide_operator() {
  this->process_operator([]() -> Operator_Node* { return new Divide_Node(); });
}
...
void Tree_Builder::build_modulus_operator() {
  this->process_operator([]() -> Operator_Node* { return new Modulus_Node(); });
}
