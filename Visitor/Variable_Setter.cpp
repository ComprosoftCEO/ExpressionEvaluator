...
// Constructor
//
Variable_Setter::Variable_Setter(const std::map<std::string, int>& lookup): lookup_(lookup) {}
...
void Variable_Setter::visit_variable_command(Variable_Command& command) {
  ...
  if (search != this->lookup_.end()) { command.set_value(search->second); }
}
...
void Variable_Setter::visit_variable_node(Variable_Node& node) {
  ...
  if (search != this->lookup_.end()) { node.set_value(search->second); }
}
